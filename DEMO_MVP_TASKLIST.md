...
- [ ] Tạo bảng `Songs`.
  ...
  - [ ] `songBookNameSnapshot` - legacy field (tuỳ chọn, không dùng để hiển thị chính).
  - [ ] `pageNumber` - số trang, kiểu number, bắt buộc.
...
- [ ] Khi submit form:
  ...
  - [ ] Lưu `songBookId` (tên tập sách hiển thị theo dữ liệu `SongBooks` hiện tại).
...
- [ ] Mỗi dòng tập sách hiển thị:
  ...
  - [ ] Số trang nhỏ nhất của các bài trong tập.
- [ ] Danh sách tập sách sắp xếp theo trang bài hát nhỏ nhất (`pageNumber`) từ nhỏ tới lớn.
...
- [ ] Chuẩn hóa model phía frontend:
  - [ ] `Song`: `id`, `title`, `firstLine`, `author`, `songBookId`, `pageNumber`, `linkPdf` (có thể giữ `songBookNameSnapshot` cho tương thích cũ).
  - [ ] `SongBook`: `id`, `name`.
...
- [ ] API tạo bài hát chỉ nhận 6 field demo:
  ...
  - [ ] `pageNumber`
  - [ ] `linkPdf`
- [ ] UI hiển thị tên tập sách theo `SongBooks` hiện tại (không phụ thuộc `songBookNameSnapshot`).
...
- [x] Hiển thị và sắp xếp tập sách theo trang bài hát nhỏ nhất (`pageNumber`) tăng dần.
...
- [x] (Phía bạn) Tạo bảng `SongBooks` với field tối thiểu:
  - [x] `id` (primary key tự sinh của NocoDB).
...
- [x] (Phía bạn) Tạo bảng `Songs` với field tối thiểu:
  ...
  - [x] `songBookId` (Single line text, bắt buộc).
  - [x] `songBookNameSnapshot` (Single line text, legacy/tuỳ chọn).
...
- [x] (Phía bạn) Chọn 1 trong 2 cách cấu hình API bảng:
  - [ ] Cách A (khuyến nghị): lấy endpoint trực tiếp và cung cấp:
    - [ ] `NOCODB_SONG_BOOKS_ENDPOINT`
    - [ ] `NOCODB_SONGS_ENDPOINT`
  - [x] Cách B: lấy Table ID và cung cấp:
...
- [x] (Phía bạn) Điền `backend/.env`:
  ...
  - [x] `NOCODB_BASE_URL=...`
  - [x] `NOCODB_API_TOKEN=...`
  - [x] endpoint hoặc table id theo Cách A/B ở trên.
