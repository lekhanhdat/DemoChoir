...
- [x] Tạo data access layer cho NocoDB bằng `httpx`.
  ...
  - [x] Mapping rõ quan hệ `Songs.songBookId` -> `SongBooks.songBookId`.
- [x] Backend chỉ chạy với NocoDB Cloud, không thêm adapter mock.
...
- [x] Hiển thị tập sách tối giản (tên tập, số bài, nút `Xem bài`) không có cột `Trang nhỏ nhất`.
...
- [x] Redeploy backend để nhận code mới (route `/`); URL root backend đã trả thông tin app.
- [x] Cập nhật frontend env var `VITE_API_BASE_URL` trên Vercel trỏ tới backend production.
...
- [ ] Smoke test deploy Vercel:
  ...
  - [ ] Frontend Vercel gọi được backend qua `VITE_API_BASE_URL` (đang bị chặn bởi CORS trên backend production).
...
- [x] Đã redeploy backend production; URL root backend đã trả thông tin app.
- [ ] Chưa chạy test `POST /api/song-books` và `POST /api/songs` trên production để tránh tạo thêm dữ liệu rác trong bảng demo.
- [ ] Frontend production đã trỏ backend Vercel nhưng chưa đọc được dữ liệu do CORS.
...
- [x] Redeploy backend Vercel để áp dụng route `/` mới.
- [ ] Chạy smoke test production cho 2 API tạo mới (`POST /api/song-books`, `POST /api/songs`) với dữ liệu kiểm thử tối thiểu.

## Cập nhật triển khai 2026-05-17

### Trạng thái production đã xác minh

- [x] `GET https://demo-choir-backend.vercel.app/api/health` trả `{ status: "ok", adapter: "nocodb", dataReady: "true" }`.
- [x] `GET https://demo-choir-backend.vercel.app/` đã trả thông tin app (không còn `404`).
- [x] `GET /api/song-books` và `GET /api/songs` trên backend production trả dữ liệu thật từ NocoDB.

### Blocker hiện tại

- [x] Xác định nguyên nhân FE production không thấy dữ liệu: backend chưa trả `Access-Control-Allow-Origin` cho origin frontend Vercel (CORS bị browser chặn).

### Đã triển khai trong code (chờ redeploy backend)

- [x] Bổ sung cấu hình `CORS_ALLOWED_ORIGIN_REGEX` trong backend settings.
- [x] Cập nhật FastAPI `CORSMiddleware` để nhận cả danh sách origin và regex origin.
- [x] Mặc định cho phép domain `*.vercel.app` bằng regex để hỗ trợ production + preview deploy.
- [x] Cập nhật `backend/.env.example` và `README.md` cho cấu hình CORS mới.

### Việc tiếp theo

- [ ] Redeploy backend Vercel để áp dụng bản sửa CORS.
- [ ] Smoke test lại frontend production: load dữ liệu ở `/`, `/songs`, `/song-books`.
- [ ] Chạy smoke test 2 API `POST` với dữ liệu kiểm thử tối thiểu rồi xóa record test trực tiếp trên NocoDB.
